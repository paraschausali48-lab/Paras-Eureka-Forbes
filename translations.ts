 ...
 *
 * @param {string} lang - Language code ('en', 'hi', 'mr' or 'gu')
 */
export function applyTranslations(lang: string): void {
  if (!translations[lang]) {
    console.warn(`[i18n] translations.ts: Language '${lang}' not found. Falling back to 'en'.`);
    lang = 'en';
  }
  ...
  elements.forEach(function (el) {
    ...
    if (tagName === 'input' || tagName === 'textarea') {
      ...
      // Safely update text without destroying child elements (like SVGs)
      if (el.children.length > 0) {
        ...
        for (let i = 0; i < el.childNodes.length; i++) {
          if (
            ...
            el.childNodes[i].nodeValue!.trim() !== ''
          ) {
        ...
        if (!textNodeFound) el.appendChild(document.createTextNode(translation));
      } else {

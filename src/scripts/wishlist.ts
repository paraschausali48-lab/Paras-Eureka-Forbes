...
export function updateWishlistUI() {
  ...
  document.querySelectorAll<HTMLElement>('.add-to-wishlist-btn').forEach((btn) => {
    const card = btn.closest('.product-card') as HTMLElement;
    if (card && card.dataset.sku) {
      const isAdded = list.includes(card.dataset.sku);
      ...
      btn.innerHTML = isAdded
        ? '<svg aria-hidden="true" width="20" height="20" fill="#e63946" stroke="#e63946" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>'
        : '<svg aria-hidden="true" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>';
    }
  });
}
...
registerClickAction({
  selector: '.add-to-wishlist-btn',
  handle: (el: HTMLElement, e: Event) => {
    e.preventDefault();
    e.stopPropagation(); // Prevents the card click from navigating to the PDP
    const card = el.closest('.product-card') as HTMLElement;
    if (card && card.dataset.sku) {
      handleWishlistToggle(card.dataset.sku);
    }
  },
});
